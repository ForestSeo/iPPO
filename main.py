		...
		while True:
			if self.is_starting:
				...
				info = (
					...
					"・残りの分身の個数は左上のアイコンの数",
					"・残りの手数制限は左上に数字で書いてある",
					"・複数あるゴールに同時にプレイヤーが存在した場合",
					"　ゴールとなる。",
					"・ゴールの判定は動いているかどうかは関係ない",
					"・Rキーでリスタート",
				)
				self.screen.blit(self.font_start.render("ENTERでスタート", True, (255, 255, 255)), (200, 180))
				for i, s in enumerate(info):
					self.screen.blit(self.font.render(s, True, (255, 255, 140)), (50, 250 + i * 20))
